...
usage = "%prog [options] <treatment GERALD alignment filename> <control GERALD alignment filename> <refSeq gene annotation file>"
...
if __name__ == '__main__' :
	...
	calls = ["split_qsub.py %s --ext=.bed gerald_to_bed.py --util-args=\"%s\" %s/*.[0-9][0-9][0-9][0-9]"%(opts.qsub_args,opts.bed_args,exp_wrk_dir),
	  ...
	  #       "sort_bed.py %s/*.bed > %s"%(exp_wrk_dir,exp_bed_fn),
	  #       "sort_bed.py %s/*.bed > %s"%(cnt_wrk_dir,cnt_bed_fn),
	        ]
