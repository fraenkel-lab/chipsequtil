...
usage = "%prog [options] <peaks fn> <fg fasta> <bg fasta> <TAMO motif fn>"
...
parser.add_option('-p','--processes',dest='n_procs',type='int',default=3,help='number of subprocesses to run concurrently [default: %default]')
...
if __name__ == '__main__' :
    ...
    if opts.motif_ind != 'all' :
        motif_indices = parse_steplist(opts.motif_ind)
    ...
    for i,m in zip(motif_indices,motifs) :
        ...
        mp.ylabel('avg(-10*log10(pvalue)')
        mp.xticks(np.arange(motif_score_bins.size)[1::5],['%.2f'%x for x in motif_score_bins[1::5]])
        mp.plot(np.arange(motif_score_bins.size-1),
                [x.mean() for x in binned_motif_scores],
                label='Foreground\n(avg. bg score=%.2g)'%m_mat[1,:].mean())
        mp.legend(loc='upper left')

        img_fn = os.path.join(out_dir,m_name.translate(fn_trans)+'_%d_peakmot.png'%i)
        mp.savefig(img_fn)
        mp.clf()
